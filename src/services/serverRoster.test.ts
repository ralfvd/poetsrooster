...
describe("serverback-up", () => {
  it("stuurt bij opslaan beide wachtwoordvelden en alle roostergegevens", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ createdAt: "2026-08-23T10:00:00Z" }), {
      status: 201,
      headers: { "Content-Type": "application/json" },
    }));
    vi.stubGlobal("fetch", fetchMock);
    await saveRosterToServer("Ralf", "lang-wachtwoord", "lang-wachtwoord", state);
