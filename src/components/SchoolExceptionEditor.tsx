...
export function SchoolExceptionEditor({ schoolFile, loadError, minDate, maxDate, onSaved, onRefresh }: Props) {
  ...
  const unlock = async () => {
    ...
    setBusy(true);
    try {
      await unlockSchoolExceptionEditor(password);
      setUnlocked(true);
      setMessage("");
    } catch (error) {
      setUnlocked(false);
      setMessage(error instanceof Error ? error.message : "Ontgrendelen mislukt.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Opslaan mislukt.";
      setMessage(errorMessage);
      if (errorMessage === "Onjuist beheerwachtwoord.") setUnlocked(false);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <details className="panel school-exceptions-panel">
      <summary className="school-admin-summary">
        <span>
          <span className="eyebrow">Alleen voor beheerders</span>
          <strong>Schoolkalender voor iedereen</strong>
        </span>
      </summary>
      {!unlocked ? (
        <form
          className="school-unlock"
          onSubmit={(event) => {
            event.preventDefault();
            void unlock();
          }}
        >
          <p className="panel-help">Vul eerst het schoolbeheerwachtwoord in om deze kalender te bekijken en te wijzigen.</p>
          <label>
          ...
          <button className="button primary full-width" type="submit" disabled={busy || !password}>
            {busy ? "Controleren…" : "Schoolkalender ontgrendelen"}
          </button>
          {message && <p className="school-message" role="status">{message}</p>}
        </form>
      ) : (
        <div className="school-admin-content">
          <div className="row-heading school-calendar-heading">
